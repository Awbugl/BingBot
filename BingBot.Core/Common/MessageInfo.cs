...
using MessageChain = BingBot.Core.Message.MessageChain;
...
public class MessageInfo
{
    ...
    private Message.MessageChain? ReplyMessages { get; set; }
    ...
    private static MessageBuilder FromMessageChain(Message.MessageChain messages)
    {
    ...
    private Task<int> SendPrivateMessage(Message.MessageChain messages) => Bot.SendFriendMessage(FromQQ, FromMessageChain(messages));

    private Task<int> SendGroupMessage(Message.MessageChain messages) => Bot.SendGroupMessage(FromGroup, FromMessageChain(messages));

    internal async Task<int> SendMessage(Message.MessageChain? message)
    {
    ...
    internal async Task<int> SendMessageOnly(Message.MessageChain? message)
    {
        if (message is null) return -1;
            ...
            foreach (KeyValuePair<(Type, Func<ExecutorBase, object?>), string[]> pair in _methodPrefixs)
            {
                ...
                try
                {
                    info.ReplyMessages = method(_ctors[executor](info)) switch
                                         {
                                             Task<Message.MessageChain> task => await task,
                                             Message.MessageChain chain      => chain,
                                             _                               => null
                                         };
                }
    ...
    private static TextMessage GetErrorMessage(Exception e) => RobotReply.OnExceptionOccured(e);
